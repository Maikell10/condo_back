...
const getOrCreateBuilding = async (connection, buildingName, buildingMap) => {
...
const upsertOwner = async (connection, ownerName, email, defaultPassword) => {
...
const main = async () => {
    ...
    try {
        ...
        const [summary] = await connection.query(
            `SELECT b.id, b.name, COUNT(a.id) AS aptos, ROUND(SUM(a.alicuota), 6) AS suma
             FROM buildings b
